...
let test prog =
  ...
  Array.iteri (fun i name ->
    if i < 2 then begin
      ...
      T.attach_test_lib_actions t ~pid ~actions:(P.Selected [(P.Enable, name)]);
      Unix.sleep 1;
      T.print_info t ~pid;
      T.attach_test_lib_actions t ~pid ~actions:(P.Selected [(P.Disable, name)]);
      Unix.sleep 1;
      T.print_info t ~pid;
    end
  ) names;
  ...
  Unix.sleep 1;
  T.print_info t ~pid;
  let actions = P.Selected
                  (Array.map (fun n -> (P.Disable, n)) names
                   |> Array.to_list) in
